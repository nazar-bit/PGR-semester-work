...
namespace vasylnaz {
	...
	void Mesh::draw(const ShaderProgram& shader_program, const glm::mat4& model, const glm::mat4& view) const {
		glBindVertexArray(vao);

		glUniformMatrix4fv(shader_program.positionM, 1, GL_FALSE, glm::value_ptr(model));
		if (shader_program.positionV != -1) {
