...
void main() {
    ...
    float coords_coef = time/4 - sin(time);
    float new_y = fg_tex_coords.y + coords_coef;
    float screen_end = 0.38;
    float screen_start = 0.22;
    while(new_y > screen_end){
        new_y = new_y - screen_end + screen_start;
    }
    vec2 new_tex_coords = vec2(fg_tex_coords.x, new_y);
    
    vec3 normMap = texture(normSampler, new_tex_coords).rgb;
    normMap = normMap * 2.0 - 1.0; 
    vec3 norm = normalize(TBN * normMap); 
    vec3 view_pos = fg_position.xyz;

    // TextureColor
    vec4 tex_clr = texture(texSampler, new_tex_coords);
    if(tex_clr.a < 0.1){
        discard;
    }
    ...
    vec3 em_tex = texture(emSampler, new_tex_coords).rgb;
    if(em_tex != vec3(0.0)){
        final_em = em_tex;  
    }
